...
function validateAppearance(value: unknown, path: string, issues: ValidationIssue[]): void {
  ...
  if (value.linetypeScale !== undefined && (
    typeof value.linetypeScale !== "number" || !Number.isFinite(value.linetypeScale) || value.linetypeScale <= 0
  )) {
  ...
  for (const key of ["plotStyleId", "materialId"] as const) {
  ...
  layouts.forEach((layout, layoutIndex) => {
    ...
    if (layout.pageSetup !== undefined) {
      ...
      if (!isRecord(layout.pageSetup)) {
        ...
        } else if (setup.plotScale.mode === "custom" && (
          typeof setup.plotScale.paperUnits !== "number" || setup.plotScale.paperUnits <= 0 ||
